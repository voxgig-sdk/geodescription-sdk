...
class LonlongitudeDirectTest extends TestCase
{
    public function test_direct_list_lonlongitude(): void
    {
        $setup = lonlongitude_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-lonlongitude", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["latitude01", "longitude01"] as $_liveKey) {
