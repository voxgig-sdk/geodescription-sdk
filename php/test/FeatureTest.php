...
// Geodescription SDK feature test
//
...
class FtCtrl extends GeodescriptionControl
{
...
class FtClient
{
    ...
    private GeodescriptionUtility $utility;

    public function __construct(GeodescriptionUtility $utility, array $options)
    {
    ...
    public function get_utility(): GeodescriptionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GeodescriptionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GeodescriptionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GeodescriptionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GeodescriptionConfig::make_feature($name);
    ...
    private static function buildUrl(GeodescriptionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GeodescriptionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GeodescriptionResponse($fetched);
        }
    ...
    private function fail(GeodescriptionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GeodescriptionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
