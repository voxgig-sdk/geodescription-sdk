...
// Geodescription SDK pipeline test
//
...
{
    private static function utility(): GeodescriptionUtility
    {
        return new GeodescriptionUtility();
    }
    ...
    private static function ctx(array $over = []): GeodescriptionContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GeodescriptionContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GeodescriptionOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GeodescriptionError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GeodescriptionOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GeodescriptionMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GeodescriptionOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GeodescriptionMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GeodescriptionOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GeodescriptionMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeodescriptionSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GeodescriptionMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GeodescriptionResponse([]),
            'result' => new GeodescriptionResult([]),
        ]);
        [, $err] = GeodescriptionMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec([]),
            'response' => null,
            'result' => new GeodescriptionResult([]),
        ]);
        [, $err] = GeodescriptionMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec(['step' => 's']),
            'response' => new GeodescriptionResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GeodescriptionResult([]),
        ]);
        [, $err] = GeodescriptionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec(['step' => 's']),
            'response' => new GeodescriptionResponse(self::resp(200, ['v' => 1])),
            'result' => new GeodescriptionResult([]),
        ]);
        [, $err] = GeodescriptionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec(['step' => 's']),
            'response' => new GeodescriptionResponse(self::resp(200, ['v' => 2])),
            'result' => new GeodescriptionResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GeodescriptionMakeResponse::call($ctx);
    ...
    {
        $preset = new GeodescriptionResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec([]),
            'response' => new GeodescriptionResponse([]),
            'result' => new GeodescriptionResult([]),
        ...
        [$got, $err] = GeodescriptionMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GeodescriptionResult([])]);
        [, $err] = GeodescriptionMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GeodescriptionSpec([]), 'result' => null]);
        [, $err] = GeodescriptionMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeodescriptionSpec(['step' => 's']),
            'result' => new GeodescriptionResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GeodescriptionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeodescriptionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeodescriptionSpec(['step' => 's']),
            'result' => new GeodescriptionResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GeodescriptionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeodescriptionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeodescriptionResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GeodescriptionSpec([]),
            'result' => new GeodescriptionResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GeodescriptionMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeodescriptionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GeodescriptionError('boom', 'boom');
        $utility->fetcher = function (GeodescriptionContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeodescriptionSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GeodescriptionMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GeodescriptionContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GeodescriptionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GeodescriptionResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GeodescriptionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GeodescriptionContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeodescriptionSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GeodescriptionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GeodescriptionResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GeodescriptionSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GeodescriptionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GeodescriptionMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GeodescriptionMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GeodescriptionResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GeodescriptionDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GeodescriptionResult(['ok' => false])]);
        $this->expectException(GeodescriptionError::class);
        GeodescriptionDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GeodescriptionResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GeodescriptionMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GeodescriptionMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GeodescriptionBaseFeature();
        $b = new GeodescriptionBaseFeature();
        GeodescriptionFeatureAdd::call($ctx, $a);
        GeodescriptionFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GeodescriptionBaseFeature
    {
        $f = new GeodescriptionBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GeodescriptionFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GeodescriptionContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GeodescriptionSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeodescriptionPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GeodescriptionPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GeodescriptionResponse(['headers' => null]),
            'result' => new GeodescriptionResult([]),
        ]);
        GeodescriptionResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GeodescriptionResultBody::call($ctx);
