...
// Geodescription SDK ReverseGeocoding entity
...
class ReverseGeocodingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ReverseGeocodingEntity($this->_client, $opts);
    }

    /**
     * @param ReverseGeocoding|array $args ReverseGeocoding data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GeodescriptionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ReverseGeocoding|array The current ReverseGeocoding data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ReverseGeocoding fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GeodescriptionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ReverseGeocoding fields).
     */
     ...
     *
     * @param ReverseGeocodingLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ReverseGeocodingLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ReverseGeocoding|array The loaded ReverseGeocoding as an assoc-array at the
     *   SDK boundary; throws GeodescriptionError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GeodescriptionHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
