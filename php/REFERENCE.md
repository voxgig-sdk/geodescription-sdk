# Geodescription PHP SDK Reference

Complete API reference for the Geodescription PHP SDK.
...
$client = new GeodescriptionSDK($options);
```
...
#### `GeodescriptionSDK::test($testopts = null, $sdkopts = null)`
...
#### `Lonlongitude($data = null)`

Create a new `LonlongitudeEntity` instance. Pass `null` for no initial data.

#### `ReverseGeocoding($data = null)`

Create a new `ReverseGeocodingEntity` instance. Pass `null` for no initial data.
