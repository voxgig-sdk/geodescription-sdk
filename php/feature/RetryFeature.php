...
// Geodescription SDK retry feature
...
class GeodescriptionRetryFeature extends GeodescriptionBaseFeature
{
    ...
    public function init(GeodescriptionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GeodescriptionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GeodescriptionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GeodescriptionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
