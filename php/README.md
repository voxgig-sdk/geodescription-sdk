...
The PHP SDK for the Geodescription API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($lonlongitudes as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $lonlongitude));
```
