...
local TextPartEntity = {}
TextPartEntity.__index = TextPartEntity


function TextPartEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TextPartEntity)
  self._name = "text_part"
  self._client = client
...
function TextPartEntity:get_name()
  return self._name
end


function TextPartEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TextPartEntity.new(self._client, opts)
end
...
function TextPartEntity:mark_deleted()
  self._deleted = true
end


function TextPartEntity:deleted()
  return true == self._deleted
end


function TextPartEntity:data_set(args)
  if args ~= nil then
...
function TextPartEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TextPartEntity:match_set(args)
  if args ~= nil then
...
function TextPartEntity:match_get()
...
function TextPartEntity:stream(action, args, callopts)
...
---@param reqmatch TextPartListMatch
---@param ctrl? table
---@return TextPart[]
---@return string? err
function TextPartEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TextPartEntity:_run_op(ctx, post_done)
  local utility = self._utility
