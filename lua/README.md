...
The Lua SDK for the Geodescription API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Lonlongitude(nil):list(nil, nil)
...
local result, err = client:Geodescription(nil):load(
  { id = "test01" }, nil
)
