...
module GeodescriptionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GeodescriptionResponse.new({})
    ...
    if fetch_err
      ...
      response = GeodescriptionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GeodescriptionResponse.new(fetched)
    else
