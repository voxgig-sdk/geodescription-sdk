...
class TextPartEntity
  ...
  def make
    opts = @_entopts.dup
    TextPartEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GeodescriptionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TextPart, Hash] the current TextPart data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GeodescriptionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TextPart fields)
  def match_get
  ...
  #
  # @param reqmatch [TextPartListMatch, Hash, nil] match filter (any subset of
  #   TextPart fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TextPart>, Array] the matching TextPart items; raises GeodescriptionError on failure
