...
class ReverseGeocodingEntity
  ...
  def make
    opts = @_entopts.dup
    ReverseGeocodingEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.ReverseGeocoding.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GeodescriptionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
