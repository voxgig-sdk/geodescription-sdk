...
require_relative 'Geodescription_types'
...
class GeodescriptionSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GeodescriptionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GeodescriptionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GeodescriptionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GeodescriptionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GeodescriptionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GeodescriptionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GeodescriptionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue GeodescriptionError => err
    ...
    if fetched.is_a?(Hash)
      status = GeodescriptionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GeodescriptionError.new(
        "graphql_error", "GeodescriptionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GeodescriptionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
