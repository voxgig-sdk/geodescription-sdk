...
class TextPartDirectTest < Minitest::Test
  def test_direct_list_text_part
    setup = text_part_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-text_part", setup[:live] ? "live" : "unit")
    if _should_skip
...
def text_part_direct_setup(mockres)
