...
class ReverseGeocodingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = reverse_geocoding_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "reverse_geocoding." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GEODESCRIPTION_TEST_REVERSE_GEOCODING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    reverse_geocoding_ref01_data = nil
    if reverse_geocoding_ref01_data_raw.length > 0
...
def reverse_geocoding_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "reverse_geocoding", "ReverseGeocodingTestData.json")
  ...
  entid_env_raw = ENV["GEODESCRIPTION_TEST_REVERSE_GEOCODING_ENTID"]
