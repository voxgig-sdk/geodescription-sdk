# Geodescription TypeScript SDK



The TypeScript SDK for the Geodescription API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Lonlongitude()` — each with a small set of operations (`list`, `load`)
...
```ts
const client = new GeodescriptionSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `GeodescriptionSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `GeodescriptionSDK.test(testopts?, sdkopts?)` | `GeodescriptionSDK` | Create a test-mode client. |
...
```ts
const reverse_geocoding = await client.ReverseGeocoding().load({ lat: 1, lon: 1 })
```
...
```ts
const text_parts = await client.TextPart().list({ lat: 1, lon: 1 })
```
...
geodescription/
├── src/
│   ├── GeodescriptionSDK.ts        # Main SDK class
