...
describe('LonlongitudeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new GeodescriptionSDK({
  ...
  test('direct-list-lonlongitude', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-lonlongitude', setup.live)) return
    if (skipIfMissingIds(t, setup, ["latitude01","longitude01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GEODESCRIPTION_TEST_LONLONGITUDE_ENTID']
