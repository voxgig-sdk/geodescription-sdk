...
describe('ReverseGeocodingDirect', async () => {
  ...
  test('direct-load-reverse_geocoding', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-reverse_geocoding', setup.live)) return
