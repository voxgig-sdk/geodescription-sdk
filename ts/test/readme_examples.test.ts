...
//     (\`new GeodescriptionSDK(...)\` / \`GeodescriptionSDK.test(...)\` become
//     \`GeodescriptionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'GeodescriptionSDK'
...
const TEST_SEED = {"entity":{"lonlongitude":{"test01":{"id":"test01"}},"reverse_geocoding":{"test01":{"id":"test01"}},"text_part":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GeodescriptionSDK(...)\` and any
// \`GeodescriptionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GeodescriptionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GeodescriptionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
