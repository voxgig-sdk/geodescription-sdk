# Geodescription SDK test runner
...
class GeodescriptionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GeodescriptionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GeodescriptionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GeodescriptionTestRunner.getenv("GEODESCRIPTION_TEST_LIVE")
        override = GeodescriptionTestRunner.getenv("GEODESCRIPTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GeodescriptionTestRunner.getenv(key)
        ...
        explain = GeodescriptionTestRunner.getenv("GEODESCRIPTION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GEODESCRIPTION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GeodescriptionTestRunner._test_control is not None:
            return GeodescriptionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GeodescriptionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GeodescriptionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GeodescriptionTestRunner.load_test_control()
...
def load_env_local():
    GeodescriptionTestRunner.load_env_local()


def env_override(m):
    return GeodescriptionTestRunner.env_override(m)


def entity_data(v):
    return GeodescriptionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GeodescriptionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GeodescriptionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GeodescriptionTestRunner.load_test_control()


def live_client_options():
    return GeodescriptionTestRunner.live_client_options()


def live_delay_ms():
    return GeodescriptionTestRunner.live_delay_ms()
