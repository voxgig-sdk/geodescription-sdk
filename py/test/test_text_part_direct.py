...
class TestTextPartDirect:

    def test_should_direct_list_text_part(self):
        setup = _text_part_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-text_part", "live" if setup["live"] else "unit")
        if _skip:
...
def _text_part_direct_setup(mockres):
