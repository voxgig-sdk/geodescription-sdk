...
This API provides a service for reverse geocoding, converting geographic coordinates into human-readable address-like location descriptions.

Learn more about GeoDescription API at [geodescription.com](https://geodescription.com).
...
$lonlongitudes = $client->Lonlongitude()->list();
print_r(array_map(fn($item) => $item->data_get(), $lonlongitudes));
```
