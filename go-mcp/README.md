...
[MCP](https://modelcontextprotocol.io) server exposing the Geodescription SDK as
two agent tools — `geodescription_list` and `geodescription_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geodescription-mcp)
make build

# 2. Provide credentials via the environment
export GEODESCRIPTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user geodescription \
  -- /absolute/path/to/geodescription-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./geodescription-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// geodescription_list: first page of records
{ "entity": "lonlongitude" }
{ "entity": "lonlongitude", "query": { } }

// geodescription_load: one record by id
{ "entity": "reverse_geocoding", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geodescription-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user geodescription \
     -- "$PWD"/dist/*/geodescription-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `geodescription_list` and `geodescription_load` tools now appear
   in new sessions. Ask the agent to *"list lonlongitude using geodescription"*
   and it calls `geodescription_list` with `{"entity":"lonlongitude"}`.
...
```sh
export GEODESCRIPTION_APIKEY=sk_live_xxx            # API key
export GEODESCRIPTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `geodescription_list` tool
...
### Call the `geodescription_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `geodescription_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `geodescription_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GEODESCRIPTION_APIKEY` | API key sent with every request. |
| `GEODESCRIPTION_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

lonlongitude | reverse_geocoding | text_part

### Smoke test via HTTP (raw JSON-RPC)

```sh
./geodescription-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"geodescription_load","arguments":{"entity":"reverse_geocoding","query":{"id":1}}}}'
```
