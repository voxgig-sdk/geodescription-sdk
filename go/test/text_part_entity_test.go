...
func TestTextPartEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.TextPart(nil)
		if ent == nil {
			t.Fatal("expected non-nil TextPartEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TextPart(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TextPart(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		for _, _op := range []string{"list"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "text_part." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		// LIST
		textPartRef01Ent := client.TextPart(nil)
		textPartRef01Match := map[string]any{}

		textPartRef01ListResult, err := textPartRef01Ent.List(textPartRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, textPartRef01ListOk := textPartRef01ListResult.([]any)
		if !textPartRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", textPartRef01ListResult)
		}

	})
}

func text_partBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "text_part", "TextPartTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read text_part test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse text_part test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GEODESCRIPTION_TEST_TEXT_PART_ENTID")
