...
import (
	...
	omni "github.com/voxgig-sdk/geodescription-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GeodescriptionError); is {
				out := map[string]any{
					"name":    "GeodescriptionError",
...
func omniCtx(arg any, client *sdk.GeodescriptionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GeodescriptionSDK); is {
			client = live
