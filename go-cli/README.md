...
boru-driven command-line client **and** interactive REPL for the Geodescription
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geodescription-cli)
make build

# 2. See usage (words, entities, env vars)
./geodescription-cli --help

# 3. Provide credentials once, via the environment
export GEODESCRIPTION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./geodescription-cli list lonlongitude
./geodescription-cli list reverse_geocoding

# 5. Override the API base URL for a single call
GEODESCRIPTION_BASE=https://api.example.com ./geodescription-cli list lonlongitude

# 6. No arguments -> interactive REPL
./geodescription-cli
geodescription> list lonlongitude
geodescription> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geodescription-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/geodescription-cli list lonlongitude
   ```
...
Run with no arguments to open a REPL (prompt `geodescription>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `lonlongitude`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/geodescription-cli`. |
...
(`./geodescription-cli <expr>`) and interactively (the REPL), and expressions compose the
